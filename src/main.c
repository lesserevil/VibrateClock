...
static void tap_handler(AccelAxisType axis, int32_t direction) {
  ...
  case ACCEL_AXIS_Z:
    ...
    for (uint i=0; i< strlen(s_buffer); i++) {
      uint32_t segs[10];
      int d = ((int)(s_buffer[i]))-48;
      APP_LOG(APP_LOG_LEVEL_DEBUG, "d: %d", d);
      if (d>=0 && d<10) {
        if (d<=5) {
          for (int j=0; j<5; j++) {
            if (j<d) {
        ...
        segs[9] = 3; // Always have the last pause be three units long
        for (int j=0; j<10; j++) {
          segments[count*10+j] = segs[j]*250;
          APP_LOG(APP_LOG_LEVEL_DEBUG, "%d", (int)segs[j]);
        }
        count++;
      }
    }
